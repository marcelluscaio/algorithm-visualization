...
function ordena(parte1, parte2) {
  ...
  while (posicaoAtualParte1 < parte1.length && posicaoAtualParte2 < parte2.length) {
    ...
    if (produtoAtualParte1.price < produtoAtualParte2.price) {
      ...
      trocas.push(produtoAtualParte1.id-1)
      posicaoAtualParte1++
    } else {
      resultado.push(produtoAtualParte2);
      trocas.push(produtoAtualParte2.id-1)
      posicaoAtualParte2++
    }
  }
  //resultado no model
  posicaoAtualParte1 < parte1.length
    ? parte1.slice(posicaoAtualParte1).forEach(item => trocas.push(item.id -1))
    : parte2.slice(posicaoAtualParte2).forEach(item => trocas.push(item.id -1));
